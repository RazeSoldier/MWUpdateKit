 ...
 *
 * This file copied from https://github.com/wikimedia/mediawiki/blob/master/includes/libs/StatusValue.php,
...
class StatusValue {
     ...
     * @param string $key
     */
    public function addSuccess(string $key)
    {
        if (isset($this->success[$key])) {
            throw new \LogicException("$key already added");
        }
     ...
     * @param string $key
     */
    public function addFail(string $key)
    {
        if (isset($this->success[$key])) {
            throw new \LogicException("$key already added");
        }
